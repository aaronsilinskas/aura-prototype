...
def test_accelerate_interpolates_speed_at_midpoint_of_timer() -> None:
    ...
    effect = Effect("test", lambda pos: pos % 1.0).add_steps([accelerate(start=0.0, end=2.0)])
    state = EffectState()
...
def test_accelerate_applies_end_speed_when_timer_completes() -> None:
    ...
    effect = Effect("test", lambda pos: pos % 1.0).add_steps([accelerate(start=0.0, end=1.0)])
    state = EffectState()
...
def test_accelerate_seeds_start_speed_from_existing_velocity_when_not_set() -> None:
    ...
    effect = Effect("test", lambda pos: pos % 1.0).add_steps([accelerate(start=None, end=2.0)])
