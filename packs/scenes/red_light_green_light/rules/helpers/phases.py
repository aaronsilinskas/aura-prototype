...
:class:`~engine.state.StateSlot` that fixes the machine key and the initial
phase, so every RLGL rule reaches the same machine the same way.
...
literal never will. All eight phases share the one machine cached under
...
rlgl_phase: StateSlot = StateSlot(
    RLGL_MACHINE_KEY, lambda s: PhaseMachine(PHASE_READY), PhaseMachine
)
