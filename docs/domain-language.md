...
### DeviceConfig
The validated value object produced by the pure `parse_device_config` parser (no `board` import, pytest-covered) — it validates and normalizes an `aura-device.json` mapping but constructs no hardware. Holds `pixels` and `buttons` each as an optional (possibly empty) list — absent or `[]` parses to `pixels=[]` / `buttons=[]` rather than raising — plus optional `ir`/`audio`/`i2c`/`accelerometer`/`haptics` sections (each `None` when absent), and raises `ValueError` naming the offending field on other malformed input. Every section object except `buttons` entries also carries a mutable `enabled` attribute — see **Component enabled toggle**; a disabled section is still parsed and validated in full, so a typo in a disabled section is caught the same as in an enabled one.
...
The optional `enabled` boolean on every hardware component config object — `ir`, `audio`, `i2c`, `accelerometer`, `haptics`, and each `pixels`-list entry — defaulting to `True` when absent. `buttons` is a bare pin-name-string list and is not gated. `enabled: false` parses to a **retained**, non-`None` section object with `enabled=False` (three states: absent/`None`, present-and-enabled, present-but-disabled) — validation runs in full regardless, so "off" is a `device_builder`-time decision, not a parse-time one. `enabled` is mutable on the parsed object so a profiler can flip it after parsing to isolate hardware without re-parsing. `device_builder` treats a disabled section the same as absent — neither built nor probed, and its driver library is never imported — except for `i2c`: `enabled: false` on `i2c` builds no bus at all, distinct from an absent `i2c` section, which still falls back to the board's default `SCL`/`SDA` pins. A bus-dependent component (matrix, accelerometer, haptics) left enabled while `i2c` is disabled hits the same declared-and-enabled-but-unreachable hard error as a missing `i2c` section.
_Avoid_: treating `enabled: false` as equivalent to omitting the section at parse time (validation still runs in full); assuming `i2c: {enabled: false}` falls back to default pins (it builds no bus at all); a non-boolean `enabled` value (raises a field-named `ValueError`)
