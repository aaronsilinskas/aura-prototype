...
### DeviceConfig
The validated value object produced by the pure `parse_device_config` parser (no `board` import, pytest-covered) — it validates and normalizes an `aura-device.json` mapping but constructs no hardware. Holds `pixels` as an optional (possibly empty) list of matrix/strip configs — absent or `[]` parses to `pixels=[]` rather than raising — plus optional `ir`/`audio`/`i2c` sections (each `None` when absent), and raises `ValueError` naming the offending field on other malformed input.
