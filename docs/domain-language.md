...
### SdSyncServer
The board-free device side of the SD-sync protocol, in `hardware/shared`; services a decoded request against one `DeviceStorage` (or `None`) over an injected `Transport`, owning no serial code. `storage is None` (no `sdcard` section) makes every verb reply with a clear `"no_storage"` response instead of raising. `read`/`write` are the verb primitives `serve_pull`/`serve_push` stream over the wire; ships with `pull` and `push`, `list` is a later verb reusing the same chunked/CRC machinery. `serve_push` accumulates its own CRC-32 of the received bytes and compares it to the client's declared one before its underlying write finalizes — a mismatch replies `"crc_mismatch"` and leaves the file already on SD untouched rather than committing a corrupt replacement.
