...
_Avoid_: duplicating the wiring or scene-name resolution at a call site; reaching into `SceneRuntime` beyond its four named slots; hand-sequencing `poll_transmits`/`receive` in `run_scene` (drive `ir.update()`)
