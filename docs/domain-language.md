...
### NetworkControls
Abstract interface for network transmit (`send_ir`, `send_radio`). Always present on `GameState`; raises unless the live implementation is injected. The receive side is covered by `NetworkEvents`. This is the **game-facing** seam — the type a rule holds via `GameState.network_controls` — and it stays send-only: it **never** gains per-tick lifecycle methods, so a rule author can never see (or wonder whether to call) transmit-pump machinery. `send_ir` returns `None` — honest fire-and-forget broadcast; whether the underlying write completed synchronously or is still in flight is a transmitter-level detail (`InfraredTransmitter.send`'s own `bool` return) that stays below this seam rather than leaking a hardware writer-type distinction onto it. The concrete `HardwareNetworkControls` (`hardware/shared/network_controls.py`) is deliberately two-faced, mirroring the shipped `AudioEffectOutput(EffectOutput, VoiceSink)` port-split: a send-command surface through this abstract base, and a **runtime-facing** per-tick transmit-lifecycle pump through the separate `TransmitPump` base (see below). The engine → hardware boundary is one-way: `engine/state.py` defines this abstract seam but imports nothing from `hardware`; only `hardware/shared/network_controls.py` implements it. The runtime reaches the pump through `DeviceHardware.transmit_pump` — a handle whose *declared type* is `TransmitPump` — never by calling `poll_transmits()` on a `NetworkControls`-typed handle (that was the pre-#608 type bypass this split closes). The protection that keeps `poll_transmits()` away from rules is the *type split* (the abstract `NetworkControls` holds no pump method at all), not convention.
_Avoid_: putting `poll_transmits()` (or any per-tick pump) on the abstract `NetworkControls` (it would read as a player-intent command and confuse rule authors — the seam is send-only); calling `poll_transmits()` on a handle declared as `NetworkControls` (reaches past the declared type even though it happens to work at runtime — reach it through `TransmitPump` instead)

### TransmitPump
Abstract runtime-facing seam declaring `poll_transmits() -> dict[str, bool]` (raises `NotImplementedError`), defined in `engine/network.py` — deliberately not in `engine/state.py`, which stays purely rule-facing. A plain base class (not `typing.Protocol`, unavailable on the constrained runtimes), the same substitute pattern as `VoiceSink`. `HardwareNetworkControls` (`hardware/shared/network_controls.py`) inherits `(NetworkControls, TransmitPump)`, so one instance answers to both faces; `DeviceHardware` holds that same instance in two slots — `network_controls` (typed `NetworkControls`, handed to `GameEngine`) and `transmit_pump` (typed `TransmitPump`, polled every tick by `run_scene`, `app/scene_runtime.py`) — assigned from the one object by the builder. `poll_transmits()` fans out across the concrete class's wired `InfraredTransmitter`s to complete in-flight writes, start the oldest queued payload (FIFO, at most one per call), and release the **IR transmit gate**, returning a `dict[str, bool]` mapping each wired emitter constant to that transmitter's post-poll busy state (so a caller can check an emitter's availability *before* building and sending a payload). That dict is pre-allocated once at construction and its values are updated in place on each call — a **live view, not a snapshot** — so `app/scene_runtime.py`'s every-tick call costs no per-tick allocation; callers must read it within the same tick, not retain it across ticks expecting a stable copy.
_Avoid_: naming the pump after the gate (e.g. `update_ir_gate` — the gate is an optional downstream collaborator, not the method's subject); a single aggregate bool for `poll_transmits()`'s busy state (multiple emitters need independent per-emitter status); allocating a fresh dict per `poll_transmits()` call (violates the no-per-tick-allocation constraint — reuse the pre-allocated dict, matching `InfraredMultiReceiver`'s scratch-list convention); putting `TransmitPump` in `engine/state.py` (that module stays purely rule-facing); reaching `poll_transmits()` through a `NetworkControls`-typed handle instead of `DeviceHardware.transmit_pump`
...
The top-level `app/` package, split along the board-free / device-only seam: `app/scene_composition.py` builds the engine/effect/scene machinery for one scene (board-free, CPython-testable) and `app/scene_runtime.py` wires that up to real hardware and drives the per-tick loop (device-only). It is the one place allowed to import both the engine's runtime machinery (`GameEngine`, `SceneManager`, `EffectManager`, …) and `hardware.*` together — everywhere else, engine code stays hardware-agnostic and hardware code stays engine-agnostic.
_Avoid_: importing `hardware.*` from `engine/`, `effects/`, `magic/`, or `packs/` (the composition layer is the only sanctioned crossing point); putting board-only code in `scene_composition.py` (it must stay CPython-importable with no `TYPE_CHECKING` guard)

### SceneRuntime / build_scene_runtime
`build_scene_runtime(hw, scene_name) -> SceneRuntime` (`app/scene_composition.py`) builds the effect/rule/scene registries, `EffectManager`, `GameEngine`, and `SceneManager`, resolves *scene_name* against the scanned `SceneRegistry` — falling back to `DEFAULT_SCENE` with a console message when it is unregistered — loads it, and returns the `SceneRuntime` bundle: an `__slots__` holder of `manager` (`SceneManager`), `effect_manager` (`EffectManager`), and `timer` (`Timer`). `run_scene` (`app/scene_runtime.py`) is the only caller — it supplies the built `DeviceHardware` and drives the bundle's per-tick loop.
_Avoid_: duplicating the registry-wiring or scene-name fallback logic at a call site instead of going through `build_scene_runtime`; reaching into `SceneRuntime` for anything beyond its three named slots
...
### DeviceHardware
The named bundle `device_builder.build_hardware` returns, consumed by examples and the scene runtime. Defined in `hardware/shared/device_hardware.py` — a pure `__slots__` data holder with no board logic, importable under CPython. `__slots__`: `outputs` (`list[EffectOutput]`), `buttons`, `accelerometer` (or `None`), `network_controls` (typed `NetworkControls`, handed to `GameEngine`), `transmit_pump` (typed `TransmitPump`, polled every tick by `run_scene` in `app/scene_runtime.py`), `ir_receiver` (or `None`, polled each tick). `network_controls` and `transmit_pump` are the *same* built `HardwareNetworkControls` (`hardware/shared/network_controls.py`) instance seen through its two declared faces — the builder constructs it once and assigns both slots from it; neither call site downcasts to reach the other face. Raw IR transmitters are an internal detail wrapped before return — the runtime never handles them directly.
...
### IR transmit gate
The shared `IrTransmitGate` (`hardware/shared/ir_transport.py`) that coordinates transmit and receive so a device never decodes its own **self-echo**. The single seam between the two otherwise-decoupled sides: transmitters *drive* it (`begin_transmit`/`end_transmit` bracketing emission), the receiver *reads* it through one query, `should_discard()` — `True` while `transmitting`, then exactly once more on the falling edge (a one-shot flush latch armed by `end_transmit`, consumed inside `should_discard` itself). Neither side references the other. The gate stays a pure coordination primitive: it knows nothing of the `PulseWriter`/`PulseReader` ports (no `is_busy()`/`read_pulse()` coupling). Public surface is four members — `begin_transmit`, `end_transmit`, the read-only `transmitting` property (a side-effect-free "is the emitter lit?" window for tests/telemetry), and `should_discard`; the falling-edge flush latch is a private internal of `should_discard`, never a public method, so "consume the flush while still transmitting" is impossible by construction (the `transmitting` branch returns before the latch is touched). The deferred-release timing is the *transmitter's* concern, not the gate's: the gate is shared across every transmitter wired to a device (LINE/CONE/AOE), so "does *this* transmitter have an unclosed `begin_transmit`" is inherently per-transmitter — `InfraredTransmitter` keeps its own `_gate_armed` flag and concentrates the arm/release into one private `_release_gate_when_idle()` (called from both `_start_write` for the blocking/inline case and `poll` for the deferred/DMA case). Created and wired inside `device_builder._setup_ir`, injected into the receiver and every transmitter; optional (absent = unchanged behaviour) and off the `DeviceHardware` surface. Durable across the blocking transmitter (brackets back-to-back) and #459's non-blocking transmitter (sets on send-start, clears from its per-tick poll) — same receiver contract either way. The ordering constraint this depends on — `NetworkControls.poll_transmits()` must run before the receiver's `receive()` each tick, so a same-tick deferred release's flush latch is visible to that tick's receive — is documentation-only (a comment at the `run_scene` call site in `app/scene_runtime.py` plus the relevant docstrings), not runtime-asserted: a rare tick-loop-restructuring mistake, not a silent production risk.
_Avoid_: a second source of truth for "transmitting" (the gate is the receiver-facing view; the transmitter is the sole driver); exposing it on `DeviceHardware`; a public `consume_flush` (fold it into `should_discard` — the receiver asks one question); calling `should_discard()` more than once per tick per receiver (consumes the one-shot flush latch); giving the gate a handle on the writer/reader ports or storing the transmitter's armed flag on the gate (would need per-transmitter keying since the gate is shared — keep it a transmitter concern)
