...
### PhaseSlot
The single per-scene typed accessor owning a phase machine's `GameState` key and initial phase (`engine/phase.py`). Every one of a scene's `PhaseRule`s, `InPhaseRule`s, and its module-level phase reference is the *same* `PhaseSlot` instance, so "same key ⇒ same `PhaseMachine`" holds by construction rather than being re-supplied per rule. Callable `(state) -> PhaseMachine`, exposing `.key`. Sharing is keyed by the underlying string, so even a stray duplicate `PhaseSlot` with the same key resolves the same cached machine — hence no load-time slot-identity check.
...
### aura-device.json
The single **required** on-device file holding all hardware configuration; a missing file raises (no built-in default). Sections: `buttons`, `ir`, `pixels`, `audio`, `i2c`, `spi`, `radio`, `accelerometer`, `haptics`. `pixels` and `buttons` are each an **optional, possibly-empty list** — absent or `[]` declares zero of that output (pixels are matrix and/or strips, at most one matrix; the runtime comes up silently with none). Bus-pin sections require their pins together: `audio` the amp's I2S pins, `i2c` `sda`/`scl`, `spi` `sck`/`mosi`/`miso`, `radio` `cs`/`reset` plus `frequency` (MHz, board-variant-specific) and `node` (`0`–`254`). `accelerometer`/`haptics` are optional objects (minimal `{}`); an unknown key in any section raises a field-named `ValueError`. Every section except `buttons` also takes an optional `enabled` boolean — see **Component enabled toggle**. Pin references are name **strings**, resolved against `board` only in the device-only builder. `ir.rx` takes **one pin string or a list** — pin count alone picks the single vs. multi receiver; only `rx` is required, each emitter (`line`/`cone`/`area_of_effect`) optional (an omitted one isn't wired). Also carries a top-level `"scene"` string — per-boot game selection read separately, **not** part of `DeviceConfig`; `resolve_scene_name` raises if it's absent, empty, or non-string (no code-level default scene).
_Avoid_: `settings.toml` (removed — CircuitPython-only, unreadable on MicroPython); keying the pixel section `output`; putting `board` pin objects in the file; adding a `scene` field to `DeviceConfig`; presence-probing the accelerometer/haptics instead of declaring the section (removed — see `device_builder`)

### DeviceConfig
The validated value object produced by the pure `parse_device_config` parser (no `board` import, pytest-covered) — it validates and normalizes an `aura-device.json` mapping but constructs no hardware. Holds `pixels` and `buttons` each as an optional (possibly empty) list — absent or `[]` parses to `pixels=[]` / `buttons=[]` rather than raising — plus optional `ir`/`audio`/`i2c`/`spi`/`radio`/`accelerometer`/`haptics` sections (each `None` when absent), and raises `ValueError` naming the offending field on other malformed input. Every section object except `buttons` entries also carries an `enabled` attribute — see **Component enabled toggle**; a disabled section is still parsed and validated in full, so a typo in a disabled section is caught the same as in an enabled one. `isolate(keep)` derives a new config with every isolatable component but `keep` disabled — the supported way to isolate a single component (e.g. for a profiler), reading the isolatable set off `DeviceConfig.__slots__` rather than a hand-maintained list.
_Avoid_: importing `board` into the parser; constructing hardware in the parser (that is `device_builder`'s job); re-hardcoding the leaf-scope key list (derive from `Scope.ALL.keys`); accepting unknown scope/emitter keys; hand-listing the isolatable components (derive from `__slots__`, as `isolate` itself does)

### Component enabled toggle
The optional `enabled` boolean on every hardware component config object — `ir`, `audio`, `i2c`, `spi`, `radio`, `accelerometer`, `haptics`, and each `pixels`-list entry — defaulting to `True` when absent. `buttons` is a bare pin-name-string list and is not gated. `enabled: false` parses to a **retained**, non-`None` section object with `enabled=False` (three states: absent/`None`, present-and-enabled, present-but-disabled) — validation runs in full regardless, so "off" is a `device_builder`-time decision, not a parse-time one. `enabled` reads as an ordinary field once parsed — isolating a single component (e.g. for a profiler) goes through `DeviceConfig.isolate(keep)`, which derives a new config rather than mutating the parsed one. `device_builder` treats a disabled section the same as absent — neither built nor probed, and its driver library is never imported — except for `i2c` and `spi`: `enabled: false` on either builds no bus at all, distinct from an absent section, which still falls back to the board's default pins (`SCL`/`SDA` for `i2c`, `board.SPI()` for `spi`). A bus-dependent component (matrix, accelerometer, haptics) left enabled while `i2c` is disabled hits the same declared-and-enabled-but-unreachable hard error as a missing `i2c` section; a declared-and-enabled `radio` left wired while `spi` is disabled or unbuildable hits the same error against the SPI bus.
_Avoid_: treating `enabled: false` as equivalent to omitting the section at parse time (validation still runs in full); assuming `i2c: {enabled: false}` or `spi: {enabled: false}` falls back to default pins (each builds no bus at all); a non-boolean `enabled` value (raises a field-named `ValueError`); writing `.enabled` after parse (use `isolate`)
...
`build_scene_runtime(hw, scene_name)` builds the registries, `EffectManager`, `GameEngine`, `SceneManager`, the `InfraredManager`, and the `RadioManager`, resolves the scene (raising `ValueError` naming the known scenes when *scene_name* isn't registered — no code-level default), loads it, and returns a `SceneRuntime` bundle of `manager`, `effect_manager`, `timer`, `ir`, and `radio`. `run_scene` is its only caller.
...
### device_builder
The device-only generic hardware builder. `build_hardware(config, board, …)` resolves pin names, constructs the configured outputs/buttons/accelerometer/haptics/audio/IR/radio, and wraps IR transmitters and the radio transport in `HardwareNetworkControls`. **Single-call**: it claims pins without deiniting, so a second call raises "pin in use." Every component is **config-gated** — built only when its section is declared *and* enabled (see **Component enabled toggle**), never probed by physical presence; a declared-and-enabled component whose chip can't be constructed (including a missing bus) raises. Builds the shared I2C and SPI buses once each from configured pins, or the board's default bus when the section is absent (see **Component enabled toggle** for `enabled: false`).
...
### DeviceHardware
The named `__slots__` bundle `build_hardware` returns — a pure data holder with no board logic, CPython-importable: `outputs`, `buttons`, `accelerometer`, `network_controls` (typed `NetworkControls`), `transmit_pump` (typed `TransmitPump`), `ir_receiver`, `radio` (typed `RadioTransport | None`). `network_controls` and `transmit_pump` are the *same* `HardwareNetworkControls` instance seen through its two faces; `radio` is the same transport that instance's `send_radio` delegates to, exposed separately for a `RadioManager` to poll for receives.
...
### RadioTransport
The board-free half-duplex radio **port** `RadioManager` and `HardwareNetworkControls.send_radio` reach the chip through. One port serves both directions because an RFM69-class chip is half-duplex — unlike IR's split `PulseReader`/`PulseWriter`. A plain base class (the `VoiceSink`/`PulseReader` substitute pattern); the live adapter is `Rfm69RadioTransport`. `receive()` returns `(from_byte, data)` or `None`, non-blocking.
_Avoid_: splitting it into send/receive ports (one chip, half-duplex); importing `adafruit_rfm69` here (that lives in the adapter)

### RadioManager
The board-free per-tick owner of the radio receive path, mirroring `InfraredManager`. Constructed with a `RadioTransport` seam that may be `None`; `update()` polls it once and exposes the result as `received`, read after the call. Unlike IR it runs **no transmit pump** (the half-duplex chip's `send_radio` is fire-and-forget) and, also unlike IR, it builds the `NetworkEvents.RadioReceived` event itself — the one place a raw `(from_byte, data)` pair gains game vocabulary.
_Avoid_: a transmit pump or pump-before-receive order (radio is half-duplex, receive-only here); a value-returning `update()` (read `received`); leaving event construction to `run_scene` (radio differs from IR here — build it in the manager)

### Rfm69RadioTransport
The live CircuitPython `RadioTransport` adapter (`hardware/circuitpython/rfm69_radio_transport.py`) wrapping `adafruit_rfm69.RFM69` — the only module that imports `adafruit_rfm69`, reached from `device_builder` through a deferred import inside `_setup_radio` so a config with no `radio` section never requires the library installed. `send` transmits with `keep_listening=True` so the chip returns to RX afterward. `receive` is non-blocking: it gates on `payload_ready` before reading, then reads `with_header=True` to recover the RadioHead header, returning the From byte as `sender` and the header-stripped payload as `data`.
_Avoid_: importing `adafruit_rfm69` anywhere else; calling `receive()`'s underlying driver without checking `payload_ready` first (blocks)
...
### InfraredManager
The board-free per-tick owner of the IR sequence: `update()` runs `poll_transmits()` on its `TransmitPump` **then** `receive()` on its `InfraredReceiver`, owning the pump-before-receive order. Constructed in `build_scene_runtime` from `DeviceHardware.transmit_pump` + `ir_receiver` (may be `None`) and held as `SceneRuntime.ir`. Results are attributes read after `update()`: `received` (this tick's packet or `None`) plus forwarded `last_signal_strength`/`last_error_margin`/`telemetry_line()`. Always pumps; receives only when a receiver is wired. Unlike `RadioManager` it does *not* build the game event — `run_scene` builds the `NetworkEvents.IRReceived`, queuing it only when a scene is active (a packet decoded with no active scene is drained-and-dropped, never left to overflow).
...
Monotonic-since-boot counters at each stage of the receive path (reader, receiver, decoder), owned end-to-end by the receiver and surfaced as an `IrTelemetrySnapshot`; a drop between adjacent counters names the lossy stage. `telemetry_line()` returns a formatted line only when a counter changed; `reset_telemetry()` zeroes the counters and the change-gate baseline together. Each counter has exactly one declared owner, via an explicit `OWNED_TELEMETRY_FIELDS` tuple next to the counters it increments and resets — `InfraredDecoder` owns `packets_started`/`packets_completed`/`preamble_reject`/`mark_reject`/`space_reject` (five), `PulseReader` owns `buffer_full_on_poll` (one), `InfraredReceiver` owns `pulses_seen`/`packets_surfaced`/`pulses_dropped_transmitting` (three, receiver-owned) — and the union of the three tuples is exactly `IrTelemetrySnapshot.FIELDS`. `InfraredSourceReceiver` (the shared base behind `InfraredSingleReceiver`/`InfraredMultiReceiver`) implements both `telemetry()` and `reset_telemetry()` once by walking its `decoders`/`readers` lists against these owner tuples — reading and resetting walk the same sources, so a source a reset forgets to zero is unrepresentable rather than a bug to catch per receiver.
_Avoid_: aggregating hit/gated counts in the scene; per-tick allocation in the no-pulse path; auto-resetting counters; `run_scene` holding telemetry state itself; re-listing the counter set in a receiver's telemetry/reset (walk the sources)
