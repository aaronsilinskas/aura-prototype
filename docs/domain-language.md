...
### SdSyncServer
The board-free device side of the SD-sync protocol, in `hardware/shared`; services a decoded request against one `DeviceStorage` (or `None`) over an injected `Transport`, owning no serial code. `storage is None` (no `sdcard` section) makes every verb reply with a clear `"no_storage"` response instead of raising. Two verbs ship: `read` is the primitive `serve_pull` streams chunk-by-chunk over the wire with a whole-file CRC-32; `list` is the primitive `serve_list` sends whole in one frame, enumerating a subtree via `DeviceStorage.walk`. `push` is a later verb.
