...
| `DeviceConfig` | `hardware/shared/device_config.py` | Validated `aura-device.json`; `pixels` is an optional (possibly empty) list of `MatrixPixelsConfig` / `NeoPixelPixelsConfig`; optional `ir` / `audio` / `i2c` (`I2CConfig`) / `accelerometer` (`AccelerometerConfig`) / `haptics` (`HapticsConfig`) sections |
