...
Per-scene heap is **not** measured here. A headless scene load (against a
``NullEffectOutput`` with no matrix buffers and no audio) reports a misleading
~2x figure because scene memory is output-coupled (#450); measure scenes in
situ on the real prop with
``examples/hardware/profiling/scene_load_profiler.py`` instead.
