...
def full_isolatable_config(matrix_config):
    ...
    # excluded buses and the excluded power section -- is declared and
    # enabled, giving `isolate` tests a config where "disabled" and "absent"
...
# ---------------------------------------------------------------------------
# Power (high-current rail enable pin) validation
# ---------------------------------------------------------------------------


def test_parse_power_section_maps_pin_active_high_and_enabled(matrix_config):
    matrix_config["power"] = {"pin": "GP28", "active_high": False, "enabled": False}

    result = parse_device_config(matrix_config)

    assert result.power is not None
    assert result.power.pin == "GP28"
    assert result.power.active_high is False
    assert result.power.enabled is False


def test_parse_absent_power_section_yields_none(matrix_config):
    result = parse_device_config(matrix_config)

    assert result.power is None


def test_parse_power_missing_pin_raises_value_error_naming_field(matrix_config):
    matrix_config["power"] = {}

    with pytest.raises(ValueError, match=r"power\.pin"):
        parse_device_config(matrix_config)


def test_parse_power_non_string_pin_raises_value_error_naming_field(matrix_config):
    matrix_config["power"] = {"pin": 28}

    with pytest.raises(ValueError, match=r"power\.pin must be a string pin name"):
        parse_device_config(matrix_config)


def test_parse_power_absent_active_high_key_defaults_to_true(matrix_config):
    matrix_config["power"] = {"pin": "GP28"}

    result = parse_device_config(matrix_config)

    assert result.power.active_high is True


def test_parse_power_non_boolean_active_high_raises_value_error_naming_field(matrix_config):
    matrix_config["power"] = {"pin": "GP28", "active_high": "yes"}

    with pytest.raises(ValueError, match=r"power\.active_high"):
        parse_device_config(matrix_config)


def test_parse_power_absent_enabled_key_defaults_to_true(matrix_config):
    matrix_config["power"] = {"pin": "GP28"}

    result = parse_device_config(matrix_config)

    assert result.power.enabled is True


def test_parse_power_enabled_false_retains_object_not_none(matrix_config):
    matrix_config["power"] = {"pin": "GP28", "enabled": False}
...
def test_parse_power_non_boolean_enabled_raises_value_error_naming_field(matrix_config):
    matrix_config["power"] = {"pin": "GP28", "enabled": "yes"}

    with pytest.raises(ValueError, match=r"power\.enabled"):
        parse_device_config(matrix_config)


def test_parse_power_enabled_false_missing_pin_still_raises_value_error(matrix_config):
    matrix_config["power"] = {"enabled": False}

    with pytest.raises(ValueError, match=r"power\.pin"):
        parse_device_config(matrix_config)


def test_parse_power_unknown_key_raises_value_error_naming_allowed_keys(matrix_config):
    matrix_config["power"] = {"pin": "GP28", "speed": "fast"}

    with pytest.raises(ValueError, match=r"power\.speed.*pin, active_high, enabled"):
        parse_device_config(matrix_config)
...
@pytest.mark.parametrize("keep", _ISOLATABLE_COMPONENTS)
def test_isolate_never_touches_i2c_spi_buttons_or_power(full_isolatable_config, keep):
    config = parse_device_config(full_isolatable_config)
    ...
    assert isolated.power.pin == config.power.pin
    assert isolated.power.active_high == config.power.active_high
    assert isolated.power.enabled == config.power.enabled


def test_isolate_never_disables_power_declared_disabled(full_isolatable_config):
    """power is excluded from isolation entirely -- even a component declared
    `enabled: false` stays exactly as declared, unlike an isolatable
    component's `enabled` field, which isolate never force-enables either."""
    full_isolatable_config["power"] = {"pin": "GP28", "enabled": False}
    config = parse_device_config(full_isolatable_config)

    isolated = config.isolate(keep="audio")

    assert isolated.power.enabled is False
...
def test_isolate_on_minimal_config_is_a_no_op_for_absent_components():
    ...
    assert isolated.power is None
    assert isolated.pixels == []


def test_isolate_unknown_keep_raises_value_error_naming_valid_choices_sorted():
    config = parse_device_config({})
    expected_choices = sorted(set(DeviceConfig.__slots__) - {"buttons", "i2c", "power", "spi"})
