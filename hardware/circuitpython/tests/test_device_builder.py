...
radio, IR, and I2C/SPI bus branches, plus the logger spine.

Verifies that build_hardware wires up audio, the DRV2605 haptic driver,
accelerometer, radio, IR, and I2C bus injection paths correctly, and narrates
each step through its injected logger. Also covers open_config_i2c, the
public bus entry point onto _setup_i2c reachable without a full
build_hardware call. Matrix (IS31FL3741) and NeoPixel pixel-branch coverage
...
narration tests) lives in test_device_builder_pixels.py (#776) — this file
still uses matrix/neopixel configs as a vehicle for non-pixel-specific
coverage (e.g. I2C bus sharing, output ordering relative to audio/haptics).
All hardware modules (board, busio, pulseio, digitalio) are patched so this
suite runs under CPython.
"""
