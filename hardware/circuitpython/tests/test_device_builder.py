"""Tests for device_builder.build_hardware — radio and IR branches, plus the
general logger spine.

Verifies that build_hardware wires up radio and IR correctly -- gating each
on i2c/spi bus availability where relevant -- and narrates each step through
its injected logger. Matrix (IS31FL3741) and NeoPixel pixel-branch coverage
...
test_device_builder_audio_haptics.py (#778); this file still uses a
neopixel-plus-audio-plus-haptics config as a vehicle for one piece of
non-pixel/non-bus/non-audio-haptics coverage that spans all three (pixels
outputs preceding audio/haptic outputs) and still covers radio's own
"declared but bus unreachable" hard-error case, since that exercises the
radio subsystem, not the bus setup itself. All hardware modules (board,
